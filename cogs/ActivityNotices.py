...
from menus import (
    ...
    CustomSelectMenu,
    LOAMenu,
    YesNoColourMenu, ActivityNoticeAdministration,
)
...
class ActivityCoreCommands:
    ...
    async def core_command_admin(self, ctx: commands.Context, request_type_object: str, victim: discord.Member):
        ...
        past_victim_notices = [item async for item in self.bot.loas.db.find({
        ...
        if view.value == "create":
            ...
            if current_notice:
                ...
                await self.core_command_request(ctx, request_type_object, duration, reason, return_bypass=True, override_victim=victim)
                return await respond(
                    embed=discord.Embed(
                        title="<:success:1163149118366040106> Request Sent",
                        description=f"This {request_type_object.upper()} Request has been sent successfully.",
                        color=GREEN_COLOR
                    )
                )
        elif view.value == "list":
            async def respond(embed: discord.Embed, custom_view: discord.ui.View | None):
                if view.stored_interaction is not None:
                    if custom_view:
                        await view.stored_interaction.followup.send(
                            embed=embed,
            ...
            return await respond(
                embed=discord.Embed(
                    title="<:success:1163149118366040106> Notice Deleted",
    ...
    async def core_command_request(self, ctx: commands.Context, request_type_object: str, duration: str, reason: str, return_bypass = None, override_victim = None, starting: str = None):
        ...
        active_author_notices = [item async for item in self.bot.loas.db.find({
            "guild_id": ctx.guild.id,
            "user_id": member.id,
            "accepted": True,
            "denied": False,
